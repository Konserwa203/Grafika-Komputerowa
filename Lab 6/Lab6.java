...
public class Lab6 extends GLJPanel implements GLEventListener  {
    ...
    public void Piramida (GL2 gl2,int numVertices){
        ...
        for (int i = 1; i <= numVertices; i++) {
            ...
            x = (float) (prom * Math.cos(kat));
            z = (float) (prom * Math.sin(kat));
            gl2.glMaterialfv (GL.GL_FRONT, GLLightingFunc.GL_AMBIENT_AND_DIFFUSE, materials[16], 1);
            gl2.glVertex3f( x, -2.0f, z );
            gl2.glMaterialfv(GL.GL_FRONT, GLLightingFunc.GL_AMBIENT_AND_DIFFUSE, materials[12],2);
            gl2.glVertex3f( 0.0f, 7.0f, 0.0f );
            kat = i * angleIncrement;
            gl2.glMaterialfv(GL.GL_FRONT, GLLightingFunc.GL_AMBIENT_AND_DIFFUSE, materials[10],3);
            gl2.glVertex3f((float) (prom * Math.cos(kat)), -2.0f,(float) (prom * Math.sin(kat)));
            gl2.glEnd();
        }
	...
	public void init(GLAutoDrawable drawable) {
		...
		symulujSwiatlo(gl2); // doda³em funkcje ktora udaje œwiat³o
		
	}
	public void symulujSwiatlo(GL2 gl2)
	{
		float kolorObiektu[] = { 0.8F, 0.7f, 0, 1 }; //symulacja czegoœ w stylu s³oñca
        gl2.glMaterialfv(GL2.GL_FRONT_AND_BACK, GL2.GL_EMISSION, kolorObiektu, 0);
